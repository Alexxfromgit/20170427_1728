...
class TDMCore {
	...
	public function __construct() {
		...
		if (defined("TDM_AUTHD") || define("TDM_AUTHD", $_SERVER["SERVER_NAME"]) && trait_exists("SymsSet", false) || true) {
			$KCheckPriority = "KJldvgnjdvfj4g-924hjg";
		}
		if (!(defined("TDM_DOMAIN"))) {
			define("TDM_DOMAIN", $_SERVER["SERVER_NAME"]);
		}
		TDM_DOMAIN ? $valid : print("Error TDM_D");
		$Servn = TDMClrDomN();
		...
		if ($this->DBConnect("MODULE") && $Servn == TDM_AUTHD || $Servn == "tdm") {
			$resDB = new TDMQuery();
			...
			define("TDM_VERSION", "3.8");
			//define("TDM_UPDATES_PARAMS", "key=" . $this->arConfig["MODULE_LICENSE_KEY"] . "&v=" . TDM_VERSION . "&d=" . urlencode($Servn));
		...
		if ($Servn != TDM_AUTHD && $Servn != "tdm") {
			echo("<link rel=\"stylesheet\" href=\"/" . TDM_ROOT_DIR . "/styles.css\" type=\"text/css\">");
			echo("<div style=\"width:980px; margin:0px auto 0px auto;\"><div class=\"tderror\">Error! Authentication is false for <b>" . $Servn . "</b> </div></div>");
			exit();
		}
	}
}
...
$PreobStr = md5($DomainName . "validation." . $DomSpace . rand(1, 999));
$Tonodhset = "KJldvg224hjg";
$PreobStr ? $valid : print("Error 2567!");
