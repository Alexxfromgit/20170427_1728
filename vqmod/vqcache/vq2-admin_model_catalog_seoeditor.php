	...
	public function gettotalSEOs($data = array()) {
			...
			$sql = "select count(*) as total from (
				select 'Product' as type, p.product_id  from " . DB_PREFIX . "product p
				  ...
				  left join " . DB_PREFIX . "url_alias u on  CONCAT('product_id=', CAST(p.product_id as CHAR)) = u.query
...
if (!empty($data['filter_custom_title'])) {
			...
			$sql.="	  				 				  
				...
				select 'Category' as type,  c.category_id from " . DB_PREFIX . "category c
				  ...
				  left join " . DB_PREFIX . "url_alias u on  CONCAT('category_id=', CAST(c.category_id as CHAR)) = u.query
			...
			$sql .=	"
				...
				select 'Information' as type,  i.information_id from " . DB_PREFIX . "information i
				  ...
				  left join " . DB_PREFIX . "url_alias u on  CONCAT('information_id=', CAST(i.information_id as CHAR)) = u.query
			...
			$sql .= "	  
				...
				select 'Manufacturer' as type, m.manufacturer_id from " . DB_PREFIX . "manufacturer m				 
				   left join " . DB_PREFIX . "url_alias u on  CONCAT('manufacturer_id=', CAST(m.manufacturer_id as CHAR)) = u.query WHERE 1=1";
	...
	public function getSEOs($data = array()) {
			...
			$sql = "select * from (
				...
				select 'Product' as type, pd.name as name, p.product_id as id, u.keyword, pd.meta_description, pd.meta_keyword, pd.custom_title, pd.tag AS tags from " . DB_PREFIX . "product p
				  inner join " . DB_PREFIX . "product_description pd on p.product_id = pd.product_id
				  left join " . DB_PREFIX . "url_alias u on  CONCAT('product_id=', CAST(p.product_id as CHAR)) = u.query
...
if (!empty($data['filter_custom_title'])) {
			...
			$sql.="
				...
				select 'Category' as type, cd.name, c.category_id, u.keyword, cd.meta_description, cd.meta_keyword, custom_title, -1 from " . DB_PREFIX . "category c
				  inner join " . DB_PREFIX . "category_description cd on c.category_id = cd.category_id				  
				  left join " . DB_PREFIX . "url_alias u on  CONCAT('category_id=', CAST(c.category_id as CHAR)) = u.query
				  where cd.language_id = '" . (int)$this->session->data['language_id'] . "'";
			...
			$sql .=	"
				...
				select 'Information' as type, id.title, i.information_id, u.keyword, -1, -1, -1, -1 from " . DB_PREFIX . "information i
				  inner join " . DB_PREFIX . "information_description id on id.information_id = i.information_id				  
				  left join " . DB_PREFIX . "url_alias u on  CONCAT('information_id=', CAST(i.information_id as CHAR)) = u.query
				  where id.language_id = '" . (int)$this->session->data['language_id'] . "'";
			...
			$sql .= "	  
				...
				select 'Manufacturer' as type, m.name, m.manufacturer_id, u.keyword, -1, -1, -1, -1 from " . DB_PREFIX . "manufacturer m				 
				   left join " . DB_PREFIX . "url_alias u on  CONCAT('manufacturer_id=', CAST(m.manufacturer_id as CHAR)) = u.query WHERE 1=1";
