...
if ((!isset($_GET['token'])) || ($_GET['token'] != $session->data['token'])) 	{
		...
		if (isset($data)) {$parameters = $data['parameters'];}
			else {$parameters['calts'] = '';}
			
		if (!isset($parameters['calts'])) {$parameters['calts'] = '';}
		...
		foreach ($query->rows as $product) {
			echo 'Generating Custom alts for <b>'.$product['pname'].' (from '.$product['cname'].')</b>: ';
			...
			echo " - ".( (!$parameters['calts']) ?" No ":"<span style=\"color:red;\">$calts</span> ")." custom alt was set;<br>";
			}
	}
