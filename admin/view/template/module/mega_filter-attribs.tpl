...
<table class="table table-hover attributes" id="<?php echo $IDX; ?>_mf-attribs-table">
	<thead>
		<tr>
			...
			<th class="text-center" width="160"><?php echo $text_collapsed_by_default; ?></th>
		...
		<?php foreach( $items as $item_group_id => $item ) { ?>
			<?php foreach( $item['childs'] as $childKey => $child ) { ?>
				<tr>
					...
					<td>
						<?php $tmpTypes = array( 'checkbox', 'radio', 'select', 'image', 'image_list_radio', 'image_list_checkbox', 'slider' ); ?>
