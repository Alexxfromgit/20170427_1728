    ...
    <table class="field-api">
        <tr>
            <td>
                ...
                <div>catalog/model/tool/simple{{field.custom ? 'custom' : 'main'}}.php</div>
            </td>
            <td>
...
<script type="text/ng-template" id="scenario">
    <div>
        ...
        <div style="margin-top:10px;"><a ng-show="!setData.byDefault" ng-click="resetScenarioToDefault()"><?php echo $l->get('text_reset_all_scenarios') ?></a></div>
    </div>
</script>
    ...
    <div>
        ...
        <div style="overflow:hidden;margin-bottom:10px;">
            <div ui-sortable ng-model="setData.rows[setData.scenario]" style="float:left;width:25%;margin-right:2%;">
                <div ng-repeat="row in setData.rows[setData.scenario]" row-type="{{row.type}}" row-id="{{row.id}}" class="set-row" ng-init="setData.selectedType = setData.selectedType ? setData.selectedType : row.type;setData.selectedId = setData.selectedId ? setData.selectedId : row.id;">
            ...
            <div style="float:left;width:73%;">
                <table class="form" ng-repeat="row in setData.rows[setData.scenario]" ng-show="selected(row.type, row.id)">
                    ...
                    <tr ng-show="row.masterField">
                        ...
                        <td ng-init="row.displayWhen = !empty(row.displayWhen) ? (isArray(row.displayWhen) ? arrayToObject(row.displayWhen) : row.displayWhen) : {};">
                    ...
                    <tr ng-show="row.type == 'field' && row.required == 2 && row.masterField">
                        ...
                        <td ng-init="row.requireWhen = !empty(row.requireWhen) ? (isArray(row.requireWhen) ? arrayToObject(row.requireWhen) : row.requireWhen) : {}">
                            <div style="overflow-y:scroll;max-height:200px;">
