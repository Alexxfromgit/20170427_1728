...
////////////////////////////////////////////////////////////////////////////////
	...
	jQuery('.scrollbox div img').live('click', function() {
...
var MFP = {
	...
	createProgress: function( text ){
		return jQuery('<div style="position:absolute; z-index:99; left: 0; top: 0; background: rgba(255,255,255,0.5);" class="mega-filter-pro">')
			...
			.width( jQuery('#mf-main-content').outerWidth(true) )
			.height( jQuery('#mf-main-content').outerHeight(true) )
		...
		var self	= this,
				...
				.append((function(){
					...
					for( i in self._langs ) {
						...
						$tc.append(jQuery('<div class="tab-pane' + ( k ? '' : ' active' ) + '" id="tab-language-' + self._row + '-' + self._langs[i].language_id + '">')
							.append(jQuery('<table class="form">')
								.append(jQuery('<tr>')
									.append(jQuery('<td><?php echo $entry_title; ?></td>'))
			...
			$tc		= jQuery('<div class="tab-content">')
				...
				.append(jQuery('<div id="tab-' + self._row + '-settings" class="tab-pane active">')
					.append(jQuery('<table class="form">')
						.append(jQuery('<tr>')
							.append('<td><?php echo $entry_layout; ?><span class="help"><?php echo $text_checkbox_guide; ?></span></td>')
						...
						.append(jQuery('<tr id="category_id-list-' + self._row + '" data-name="category-list">')
							...
							.append(jQuery('<td>')
								.append( self.createScrollbox( 'delete', '[category_id][]', typeof self._categories[self._row] == 'undefined' ? [] : self._categories[self._row] ) )
							)
						)
						...
						.append(jQuery('<tr id="category_id-hide-list-' + self._row + '" data-name="category-hide-list">')
							.append('<td><span class="help"><?php echo $text_checkbox_guide; ?></span></td>')
							.append(jQuery('<td>')
								.append( self.createScrollbox( 'delete', '[hide_category_id][]', typeof self._hideCategories[self._row] == 'undefined' ? [] : self._hideCategories[self._row] ) )
							)
						)
						.append(jQuery('<tr>')
	...
	_initModule: function( $module, data ) {
		...
		$module.find('[data-name="customer-groups"]')
			.append( self.createScrollbox( 'checkbox', '[customer_groups][]', self._customerGroups, 'customer_group_id', typeof data['customer_groups'] != 'undefined' ? data['customer_groups'] : [] ) );
		...
		$module.find('input[name="category-name"]:not([autocomplete])').each(function(){
			...
			jQuery(this).attr('autocomplete','1').autocomplete({
				...
				source: function(request, response) {
					jQuery.ajax({
						url: 'index.php?route=module/mega_filter/category_autocomplete&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request.term)+MF_AJAX_PARAMS,
						dataType: 'json',
				...
				select: function(event, ui) {
					...
					$c.append('<div>' + ui.item.label + '<img src="view/image/delete.png" alt="" /><input type="hidden" name="<?php echo $_name; ?>_module[category_id][]" value="' + ui.item.value + '" /></div>');
		...
		$module.find('input[name="category-hide-name"]:not([autocomplete])').each(function(){
			...
			jQuery(this).attr('autocomplete','1').autocomplete({
				...
				select: function(event, ui) {
					...
					$c.append('<div>' + ui.item.label + '<img src="view/image/delete.png" alt="" /><input type="hidden" name="<?php echo $_name; ?>_module[hide_category_id][]" value="' + ui.item.value + '" /></div>');
	...
	createCategories: function( data ) {
		...
		function add( type, data ) {
			...
			if( type == 'related' ) {
				tbody.append(jQuery('<tr>')
					...
					.append(jQuery('<td>')
						...
						.append( self.createField( 'text', null, typeof data == 'undefined' || typeof self._categoriesNames[data.root_category_id] == 'undefined' ? '' : self._categoriesNames[data.root_category_id] ).autocomplete({
							delay: 500,
							source: function( request, response ) {
								jQuery.ajax({
									url: 'index.php?route=catalog/category/autocomplete&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request.term) + MF_AJAX_PARAMS,
									dataType: 'json',
									success: function(json) {		
										response(jQuery.map(json, function(item) {
											return {
			...
			cnt.append(jQuery('<table class="table" data-type="' + type + '">')
				.append(jQuery('<thead>')
					.append(jQuery('<tr>')
	...
	createScrollbox: function( type, name, items, key, values ) {
		...
		for( i in items ) {
			...
			switch( type ) {
				...
				case 'delete' : {
					$div.append( items[i] )
					$div.append( '<img src="view/image/delete.png">' );
		...
		if( type == 'checkbox' ) {
			$div.append(jQuery('<a onclick="jQuery(this).parent().find(\':checkbox\').attr(\'checked\', true).trigger(\'change\');">')
				.text( '<?php echo $text_select_all; ?>' )
			)
			.append( ' / ' )
			.append(jQuery('<a onclick="jQuery(this).parent().find(\':checkbox\').attr(\'checked\', false).trigger(\'change\');">')
