						...
						if( _TYPE_ == 'images' ) {
							...
							for( i = 0; i < json.length; i++ ) {
								$cnt2.find('tbody').append(jQuery('<tr>')
									...
									.append(jQuery('<td>')
										.append(jQuery('<div class="image">')
											.append(jQuery('<img src="' + ( json[i].img ? json[i].img : '<?php echo $no_image; ?>' ) + '" alt="" id="thumb-' + json[i].key + '" />'))
											.append(jQuery('<input type="hidden" name="image[' + json[i].key + ']" value="" id="image-' + json[i].key + '" />'))
											.append('<br />')
											.append('<a onclick="image_upload(\'image-' + json[i].key + '\', \'thumb-' + json[i].key + '\');"><?php echo $text_browse; ?></a>')
											.append('&nbsp;&nbsp;|&nbsp;&nbsp;')
											.append('<a onclick="jQuery(\'#thumb-' + json[i].key + '\').attr(\'src\', \'<?php echo $no_image; ?>\'); jQuery(\'#image-' + json[i].key + '\').attr(\'value\',\'\');"><?php echo $text_clear; ?></a>')
										)
									)
								);
							}
		...
		jQuery('#dialog').dialog({
			...
			close: function (event, ui) {
				if (jQuery('#' + field).attr('value')) {
					jQuery.ajax({
						url: 'index.php?route=common/filemanager/image&token=<?php echo $token; ?>&image=' + encodeURIComponent(jQuery('#' + field).val())+MF_AJAX_PARAMS,
						dataType: 'text',
