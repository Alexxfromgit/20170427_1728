...
class ControllerModuleMegaFilter extends Controller {
	...
	private function _init( $tab ) {
		...
		if( ! $curr_ver || version_compare( $curr_ver, $this->_version, '<' ) || $this->_isOldMFilterPlus() ) {			
			...
			if( $curr_ver ) {
				...
				$this->redirect($this->url->link('module/' . $this->_name . '/about', 'token=' . $this->session->data['token'], 'SSL'));
			}
	...
	public function attributes() {
		...
		if( $this->request->server['REQUEST_METHOD'] == 'POST' && $this->checkPermission() ) {
			...
			$this->redirect($this->url->link('module/' . $this->_name . '/attributes', 'token=' . $this->session->data['token'] . '&page=' . $page, 'SSL'));
		}
	...
	public function options() {
		...
		if( $this->request->server['REQUEST_METHOD'] == 'POST' && $this->checkPermission() ) {
			...
			$this->redirect($this->url->link('module/' . $this->_name . '/options', 'token=' . $this->session->data['token'] . '&page=' . $page, 'SSL'));
		}
	...
	public function filters() {
		...
		if( $this->request->server['REQUEST_METHOD'] == 'POST' && $this->checkPermission() ) {
			...
			$this->redirect($this->url->link('module/' . $this->_name . '/filters', 'token=' . $this->session->data['token'] . '&page=' . $page, 'SSL'));
		}
	...
	public function clearcache() {
		...
		$this->redirect($this->url->link('module/mega_filter/settings', 'token=' . $this->session->data['token'], 'SSL'));
	}
	...
	public function uninstall() {
		...
		$this->db->query("
			DELETE FROM
				`" . DB_PREFIX . "setting`
			WHERE
				`key` IN('mega_filter_module','mfilter_version','mega_filter_attribs','mega_filter_settings','mega_filter_filters','mega_filter_options','mfilter_plus_version','mfilter_mijoshop') OR
