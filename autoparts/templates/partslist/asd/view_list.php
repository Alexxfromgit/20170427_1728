...
<table class="tdlist">
	...
	foreach($arResult['PARTS'] as $NumKey=>$arPart){
		...
		<td>
			...
			<?}?>
			<div class="life">
			<a style="background:#FFF3B5;" href="javascript:jivo_api.open();" rel="external" class="lookup_analogues">Уточнить наличие</a><br>
			</div>
			<?if($arPart["PROPS_COUNT"]>3){?>
		...
		<td style="padding:0px;">
			<?if($arPart["PRICES_COUNT"]>0){?>
					...
					<tr class="trow <?=$HClass?> <?=$TopBord?>" <?=$HStyle?> >
						...
						<td class="tocart">
							...
							<?}else{?>
								...
								<input class="rnd-link-i" type="submit" href="javascript:void(0)" class="tdcartadd" OnClick="TDMAddToCart('<?=$arPrice['PHID']?>'); yaCounter33380443.reachGoal('addToCart_nk'); return true; ga('send', 'event', 'Knopka', 'Podtverdit')" value="Купить" ></input>
								</span>
				...
				if($arPart["PRICES_COUNT"]>$arResult['LIST_PRICES_LIMIT']){?>
					...
					<div class="moreprops2"><a href="javascript:void(0)" OnClick="ShowMoreListPrices('<?=$arPart['PKEY']?>')" class="sbut sb<?=$arPart['PKEY']?>">&#9660; <?=Lng('Show_more_prices',1,0)?> (<?=($arPart["PRICES_COUNT"]-$arResult['LIST_PRICES_LIMIT'])?>)</a></div><?
				}
			}elseif($arResult['ALLOW_ORDER']==1){?>
					<a href="javascript:jivo_api.open();" class="tdorder"><?=Lng('Order',1,0)?>
