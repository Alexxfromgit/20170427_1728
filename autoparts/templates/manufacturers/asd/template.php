...
<?if(count($arResult['CARS'])>0){
	...
	foreach($arResult['CARS'] as $arElem){
		if(in_array($arElem['NAME'],$arResult['FAVORITE'])){
			$FAV_CARS .= '<a href="'.$arElem['LINK'].'/" class="fav_logo carsbuts" style="background:url(/'.TDM_ROOT_DIR.'/media/brands/90/'.$arElem['LOGO'].'.png)" title="'.$arElem['NAME'].'">
				<div class="tdmbut-text">'.$arElem['NAME'].'</div></a>';
		}else{
			$CARS .= '<a href="'.$arElem['LINK'].'/" class="fav_logo carsbuts" style="background:url(/'.TDM_ROOT_DIR.'/media/brands/90/'.$arElem['LOGO'].'.png)" title="'.$arElem['NAME'].'">
				<div class="tdmbut-text">'.$arElem['NAME'].'</div></a>';
		}
	}
}
if(count($arResult['TRUCKS'])>0){
	...
	foreach($arResult['TRUCKS'] as $arElem){
		if(in_array($arElem['NAME'],$arResult['FAVORITE'])){
			...
			$TRUCKS .= '<a href="'.$arElem['LINK'].'-trucks/" class="fav_logo" style="background:url(/'.TDM_ROOT_DIR.'/media/brands/90/'.$arElem['LOGO'].'.png)" title="'.$arElem['NAME'].'"></a>';
		}
	}
}
?>
