...
$(document).ready(function(){  
	$('.tdm_content .secnode:nth-child(4n+1)').addClass('last');  
});
</script>
<script>
function goBack() {
...
<div class="title">
	...
	<h1><?=TDM_H1?></h1> 
	<a href="javascript:goBack()" class="bglink">&#8592; Назад к выбору комплектации модели</a> 
</div>   
...
<?if($arResult['CNT']>0){?>
	<?foreach($arResult['SECTIONS'] as $arSec){?>
		<div class="secnode" title="<?=$arSec['TITLE']?>" style="background-image:url(<?=$arSec['PICTURE']?>);">
			<a href="<?=$arSec['URL']?>" ><b><?=$arSec['NAME']?></b></a>
		</div>
	<?}?>
<?}else{?>
	<br><br>
