 ...
 *  @package    Controls
 */
class Zebra_Form_Timerange extends Zebra_Form_Control
{
    ...
    function __construct($id, $default = '', $attributes = '')
    {
        ...
        /*$time = explode('-', trim(strtolower($default)));
    ...
    function toHTML()
    {
        ...
        $attributes = $this->get_attributes(array('name', 'value', 'class', 'format', 'hours', 'hours2', 'minutes', 'from_label', 'to_label'));
        ...
        $time = explode('-', trim(strtolower($attributes['value'])));
        /*if (!isset($time[0]) || $time[0] === '')
            $time[0] = '';
        if (!isset($time[1]) || $time[1] === '')
            $time[1] = '';*/
        ...
        $output .= '<span class="timerangespan">'.$attributes['from_label'].'</span>
            <select name="' . $attributes['name'] . '_hours1" id="' . $attributes['name'] . '_hours1" ' . $this->_render_attributes() . '>
                <option value="">-</option>';
        ...
        foreach ($attributes['hours'] as $hour)
            ...
            if (!isset($ampm) || ($hour > 0 && $hour < 13))
                ...
                $output .= '<option value="' . str_pad($hour, 2, '0', STR_PAD_LEFT) . '"' . (isset($selected_hour1) && ltrim($selected_hour1, '0') == ltrim($hour, '0') ? '  selected="selected"' : '') . '>' . str_pad($hour, 2, '0', STR_PAD_LEFT) . '</option>';
        ...
        // generate the hour picker
        $output .= '<span class="timerangespan">'.$attributes['to_label'].'</span>
            <select name="' . $attributes['name'] . '_hours2" id="' . $attributes['name'] . '_hours2" ' . $this->_render_attributes() . '>
                <option value="">-</option>';
        ...
        foreach ($attributes['hours'] as $hour)
            ...
            if (!isset($ampm) || ($hour > 0 && $hour < 13))
                ...
                $output .= '<option value="' . str_pad($hour, 2, '0', STR_PAD_LEFT) . '"' . (isset($selected_hour2) && ltrim($selected_hour2, '0') == ltrim($hour, '0') ? '  selected="selected"' : '') . '>' . str_pad($hour, 2, '0', STR_PAD_LEFT) . '</option>';
