...
class Zebra_Form_Control extends XSS_Clean
{
    ...
    function get_submitted_value()
    {
        ...
        if ($attribute['locked'] !== true) {
            ...
            if (
                ...
                else if ($attribute['type'] == 'timerange') {
                    $combined = (isset($method[$attribute['name'] . '_hours1']) ? $method[$attribute['name'] . '_hours1'] : '');
                    $combined .= '-';
                    $combined .= (isset($method[$attribute['name'] . '_hours2']) ? $method[$attribute['name'] . '_hours2'] : '');

                    $method[$attribute['name']] = $combined;
                    
                    // unset the three temporary fields as we want to return to the user the result in a single field
                    // having the name he supplied
                    unset($method[$attribute['name'] . '_hours1']);
                    unset($method[$attribute['name'] . '_hours2']);
     ...
     *  where
     *
     *  -   <i>upload_path</i> the <b>URL</b> where to upload the file to - so, <i>full web path</i>, not server path,
     *      nor a relative path! "process.php" and the "_upload" method in Zebra_Form.php will automatically determine
     *      the correct <b>server path</b> where to upload the files to. Still, being behind a reverse-proxy will require
     *      manual adjustment of the logic used both in "process.php" and in the "_upload" method in "Zebra_Form.php";
     *
     *      <code>
     *      // locally this would be
     *      'http://localhost/path/to/upload-folder/'
     *
     *      // or on the Internet
     *      'http://mywebsite/path/to/upload-folder/'
     *      </code>
     *
